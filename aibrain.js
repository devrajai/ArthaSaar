/* aibrain.js - AI Brain: mood meter (news sentiment), crash warning system, TimesFM report card, sector rotation. */
(function () {
  ...
  function loadRisk() {
    ...
    function done() {
      ...
      var st = score < 25 ? ["🟢 SAB CLEAR", "#22c55e", "koi khaas khatra nahi"] :
               (score < 50 ? ["🟡 CAUTION", "#eab308", "dhyan rakho - kuch signals weak hain"] :
               (score < 75 ? ["🟠 ALERT", "#f97316", "risk badh raha hai - position size chhota rakho"] :
                            ["🔴 DANGER", "#ef4444", "multiple warnings - capital bachao"]));
      var h = '<div class="subhead">🚨 Crash Warning System <span style="float:right;color:' + st[1] + '">' + st[0] + "</span></div>" +
  ...
  function loadAcc() {
    ...
    jload("timesfm_forecasts").then(function (d) {
      var a = d.accuracy || {};
      var h = '<div class="subhead">🎓 TimesFM Report Card <span class="note" style="float:right">' + esc(d.model || "TimesFM") + "</span></div>";
      if (a.tracked) {
  ...
  function loadRot() {
    ...
    jload("timesfm_forecasts").then(function (d) {
      ...
      var h = '<div class="subhead">🔄 Sector Rotation - AI ranked</div>';
